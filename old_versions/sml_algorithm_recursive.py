...
print('\tTrain:')
print('\t\tr2: ',r2_score(y_train,predictions(parent,X_train)))
print('\t\tMSE: ',mean_squared_error(y_train,predictions(parent,X_train)))
print('\tTest:')
print('\t\tr2: ',r2_score(y_test,predictions(parent,X_test)))
print('\t\tMSE: ',mean_squared_error(y_test,predictions(parent,X_test)))


#print('\nLinear Model SML r2:',r2_score(y_test,prediction))
parent = sml(x,y,LinearRegression(n_jobs=-1),1,100)
lr = LinearRegression(n_jobs=-1).fit(x,y)
fig = plt.figure('Plot Data + Regression',figsize=(16, 12), dpi=80, facecolor='w', edgecolor='k')
ax1 = fig.add_subplot(111)
ax1.scatter(x, y, c='b', s=3, label='data')
ax1.scatter(x,lr.predict(x), c='g', s=3, label='linear r.')
ax1.scatter(x, predictions(parent,x), c='r', s=3, label='sml')
ax1.set_xlabel('x')
ax1.set_ylabel('y')
ax1.set_title('')
ax1.legend(loc=2)
