...
print("--- %s seconds ---" % round((time.time() - start_time),4))
print('\tTrain:')
print('\t\tr2: ',r2_score(y_train,sv.predict(X_train)))
print('\t\tMSE: ',mean_squared_error(y_train,sv.predict(X_train)))
print('\tTest:')
print('\t\tr2: ',r2_score(y_test,sv.predict(X_test)))
print('\t\tMSE: ',mean_squared_error(y_test,sv.predict(X_test)),'\n')

print('SML (SVR)')
start_time = time.time()
parent = sml(X_train,y_train,SVR(),8,100)
print("--- %s seconds ---" % round((time.time() - start_time),4))
pred_train = predictions(parent,X_train)
pred_test = predictions(parent,X_test)
pred_total=predictions(parent,x)
